...
function velvetblues_wmc_add_meta() {
	add_meta_box( 'comment-info', __('Comment Options', 'vb-wp-move-comments'), 'velvetblues_wmc_meta_box', 'comment', 'normal' );
}
...
<tbody>
	<tr class="alternate">
		<td class="textright">
			<label for="vb_comment_post_id"><?php _e('Page/Post ID', 'vb-wp-move-comments'); ?></label>
		</td>
		<td>
	...
	<tr>
		<td class="textright">
			<label for="vb_comment_parent_id"><?php _e('Comment Parent ID', 'vb-wp-move-comments'); ?></label>
		</td>
		<td>
	...
	<tr class="alternate">
		<td class="textright">
			<label for="vb_comment_user_id"><?php _e('Comment Author ID', 'vb-wp-move-comments'); ?></label>
		</td>
		<td>
...
function velvetblues_wmc_links( $links, $file ) {
	if ( strpos( $file, 'vb-wp-move-comments.php' ) !== false ) {
		$links = array_merge( $links, array( '<a href="http://www.velvetblues.com/web-development-blog/wp-plugin-wordpress-move-comments/" title="Need help?">' . __('Support', 'vb-wp-move-comments') . '</a>' ) );
		$links = array_merge( $links, array( '<a href="http://www.velvetblues.com/go/wpmovecommentsdonate/" title="Support plugin development">' . __('Donate', 'vb-wp-move-comments') . '</a>' ) );
	}
	return $links;
}
...
function velvetblues_wmc_quick_edit_menu($str, $input) {
    ...
    $content .= '
            <div id="replyhead" style="display:none;"><h5>'.__('Reply to Comment', 'vb-wp-move-comments').'</h5></div>
            <div id="addhead" style="display:none;"><h5>'.__('Add new Comment', 'vb-wp-move-comments').'</h5></div>
            <div id="edithead" style="display:none;">';
             
    $content .= '  
                <div class="inside">
                <label for="author">'.__('Name', 'vb-wp-move-comments').'</label>
                <input type="text" name="newcomment_author" size="50" value="" id="author" />
                </div>
         
                <div class="inside">
                <label for="author-email">'.__('E-mail', 'vb-wp-move-comments').'</label>
                <input type="text" name="newcomment_author_email" size="50" value="" id="author-email" />
                </div>
         
                <div class="inside">
                <label for="author-url">'.__('URL', 'vb-wp-move-comments').'</label>
    ...
    $content .= '
                <div class="inside">
                <label for="comment-post-id">'.__('Page/Post ID', 'vb-wp-move-comments').'</label>
                <input type="text" id="comment-post-id" name="vb_comment_post_id" size="50" value="" />
                </div>
 
                <div class="inside">
                <label for="comment-parent">'.__('Comment Parent ID', 'vb-wp-move-comments').'</label>
                <input type="text" id="comment-parent" name="vb_comment_parent_id" size="50" value="" />
                </div>
 
                <div class="inside">
                <label for="comment-author-id">'.__('Comment Author ID', 'vb-wp-move-comments').'</label>
    ...
    $content .= '          
            ...
            <span id="addbtn" style="display:none;">'.__('Add Comment', 'vb-wp-move-comments').'</span>
            <span id="savebtn" style="display:none;">'.__('Update Comment', 'vb-wp-move-comments').'</span>
            <span id="replybtn" style="display:none;">'.__('Submit Reply', 'vb-wp-move-comments').'</span></a>
			<a href="#comments-form" class="cancel button-secondary alignleft">'.__('Cancel', 'vb-wp-move-comments').'</a>
...
function velvetblues_wmc_quick_edit_action($actions, $comment ) {
    global $post;
    $actions['quickedit'] = '<a onclick="commentReply.close();if (typeof(expandedOpen) == \'function\') expandedOpen('.$comment->comment_ID.');commentReply.open( \''.$comment->comment_ID.'\',\''.$post->ID.'\',\'edit\' );return false;" class="vim-q" title="'.esc_attr__( 'Quick Edit' , 'vb-wp-move-comments').'" href="#">' . __( 'Quick&nbsp;Edit' , 'vb-wp-move-comments') . '</a>';
    return $actions;
}
